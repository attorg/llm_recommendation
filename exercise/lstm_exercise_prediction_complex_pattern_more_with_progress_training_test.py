...
split_idx = int(0.7 * len(X_exercise))  # 70% for training, 30% for testing

# Sequential split
x_train_exercise = X_exercise[:split_idx]
x_test_exercise = X_exercise[split_idx:]
x_train_injury = X_injury[:split_idx]
x_test_injury = X_injury[split_idx:]
x_train_progress = X_progress[:split_idx]
x_test_progress = X_progress[split_idx:]
y_train = y[:split_idx]
y_test = y[split_idx:]
...
# Save the model
model.save('lstm_exercise_prediction_complex_pattern_more_with_progress_w_3_test.h5')
